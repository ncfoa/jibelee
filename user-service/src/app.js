...
app.use(security.validateContentType(['application/json', 'application/x-www-form-urlencoded', 'multipart/form-data']));
...
app.get('/health', security.secureHealthCheck, async (req, res) => {
  try {
    ...
    try {
      ...
      const storageHealthy = await testS3Connection();
      health.services.storage = storageHealthy ? 'healthy' : 'unhealthy';
      if (!storageHealthy) {
        health.status = 'degraded';
      }
    } catch (error) {
...
const startServer = async () => {
  try {
    logger.info('Starting P2P Delivery User Management Service...');
    ...
    try {
      const { testS3Connection } = require('./config/storage');
      const storageConnected = await testS3Connection();
      if (!storageConnected) {
        logger.warn('Storage connection failed - file uploads may not work properly');
      }
    } catch (error) {
      logger.warn('Storage service not configured - file uploads will not work');
    }
